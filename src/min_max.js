...
function getNextBestMove(gameBoard) {
    ...
    for (const nextAction of nextActionPossiblePositions) {
        ...
        const result = minMaxWithAlphaBetaPruning(gameboardCopy, depth, nextAction, isMaximizing, -9999999999, 9999999999, true);
...
function generateTreeVisualization(gameBoard, depth = 3) {
    ...
    for (const action of possibleActions) {
        ...
        if (success) {
            const childNode = buildMinMaxTree(childBoard, depth - 1, action, childBoard.currentPlayer === 2, -9999999, 9999999, rootNode);
...
function buildMinMaxTree(gameBoard, depth, initialAction, maximizingPlayer, alpha, beta, parent) {
    /**
    ...
    if (maximizingPlayer) {
        ...
        for (const nextAction of nextActionPossiblePositions) {
            ...
            const childNode = buildMinMaxTree(gameboardCopy, depth - 1, nextAction, nextMaximizing, alpha, beta, currentNode);
...
function minMaxWithAlphaBetaPruning(gameBoard, depth, initialAction, maximizingPlayer, alpha, beta, applyInitialAction) {
    /**
    ...
    if (maximizingPlayer) {
        ...
        for (const nextActionPosition of nextActionPossiblePositions) {
            ...
            const nextMaximizing = gameboardCopy.currentPlayer === 2;
            const result = minMaxWithAlphaBetaPruning(gameboardCopy, depth - 1, initialAction, nextMaximizing, alpha, beta, false);
